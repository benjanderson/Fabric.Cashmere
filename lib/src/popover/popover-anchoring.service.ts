...
import { SatPopoverHorizontalAlign, SatPopoverVerticalAlign, SatPopoverScrollStrategy } from './popover-types';
...
export class PopoverAnchoringService implements OnDestroy {
    ...
    _overlayRef: OverlayRef | null | undefined;

    /** Reference to the target popover. */
    private _popover: PopoverContentComponent;
    ...
    anchor(popover: PopoverContentComponent, viewContainerRef: ViewContainerRef, anchor: ElementRef): void {
    ...
    private _addFallbacks(
        ...
        fallbacks.slice(1, fallbacks.length).forEach(({ h, v }) => this._applyFallback(strategy, h, v));
    }
...
function prioritizeAroundTarget<T>(target: T, options: T[]): T[] {
    ...
    const reordered: T[] = [target];

    // Make left and right stacks where the highest priority item is last
    const left: T[] = options.slice(0, targetIndex);
    const right: T[] = options.slice(targetIndex + 1, options.length).reverse();

    // Alternate between stacks until one is empty
    while (left.length && right.length) {
        const rightPop = right.pop();
        if (rightPop) {
            reordered.push(rightPop);
        }

        const leftPop = left.pop();
        if (leftPop) {
            reordered.push(leftPop);
        }
    }
